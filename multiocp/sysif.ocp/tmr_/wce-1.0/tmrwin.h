...
static void CALLBACK tmr_clock_win(HWND hwnd, UINT msg,
				   UINT timerId, DWORD curtime);

static int ntime = 1;		/* timer ID */
static HWND hwnd;
...
tmr_clock()
{
    ...
    {
	/* Don't interrupt critical section in progress */
...
TMR_startClockInterruptPlus(void (* pfHandler)(void))
{
    ...
    /* Start a timer tick going */
    if ((ntime = SetTimer((HWND) NULL,
			  (UINT) 0,
			  (UINT) tmr_millisecondsPerTick,
			  (TIMERPROC) tmr_clock_win)) == 0) {
	EH_fatal("TMR_startClockInterruptPlus: SetTimer failed\n");
    }
    if (atexit(TMR_stopClockInterrupt)) {
	EH_fatal("TMR_startClockInterruptPlus: atexit failed\n");
    }
}
...
{
    KillTimer((HWND) NULL, ntime); 
}


static void CALLBACK 
tmr_clock_win(HWND hwnd, UINT msg, UINT timerId, DWORD curtime)
{
    OS_isrActive = TRUE;
    tmr_clock();
    OS_isrActive = FALSE;
