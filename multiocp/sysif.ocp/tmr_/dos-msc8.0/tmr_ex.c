...
main(int argc, char **argv)
{
    ...
    clockPeriod = TMR_clockValidate( clockPeriod );
...
Void samplePureCpu()
{
    ...
    tmrDesc = TMR_create((TMR_MSecs)samplePeriod,IDLE_tmrHandler);
...
Void IDLE_init()
{
    ...
    tmrId = (TmrId *) TMR_getData(tmrDesc);
    *tmrId = IDLE;
    SCH_submit(IDLE_action, (Ptr)0,0,"in IDLE_init()");
}

/*<
 * Function:
...
{
    ++idleCpu;
    SCH_submit(IDLE_action, (Ptr)0,0,"in IDLE_action()");
}

/*<
 * Function:
...
{
    loadFactor = load;
    SCH_submit(LOAD_action, (Ptr)0,0,"in LOAD_init()");
}

/*<
 * Function:
...
Void LOAD_action()
{
    ...
    SCH_submit(LOAD_action, (Ptr)0,0,"in LOAD_action()");
}
