...
/*
 * SCCS Revision: @(#)du.h	1.1    Released: 10/6/87
 */
...
/*
 * Transport Specifice Data, Must be coordinated with transport */
typedef struct tp_DuInfo {
    LgInt	seqNu;
    LgInt	xpdSeqNu;
    LgInt	time;
    Uns		flags;
...
typedef struct DU_Elem {
    ...
    /* 
     * NOTYET
     * This can be simplified to get rid of the union.
     * But it must be done at the same time that the TP4 code is updated.
     * TP4 software heavily relies on uInfo.tp_d.
     */
    union {
#define DU_MAXUSERINFO 48
	/* Be very careful about DU_MAXUSERINFO.
	 * it MUST by the largest data size used by any 
	 * DU_ module user. 
