...
/*
 * If an attribute type has the PROFILE_MULTIVALUE bit set, the attribute name
 * will have a space and sequential numbers appended to it, in order to create
 * a list of values for that attribute.  E.g., if, for the attribute ATTR_ID,
 * PROFILE_addAttribute() is called as follows:
 *
 ...
 *                                   compareFunction);
 *
 * When PROFILE_searchByAttribute() is called, multiple attributes will be
 * searched for.  The attribute names will be:
 *
 *         Id 1
 *         Id 2
 *         ...
 *
 * The search will complete when a match is found, or no more attributes in
 * the set of values are found.
 */
#define	PROFILE_MULTIVALUE		((OS_Uint32)0x80000000)


/* Maximum length of an attribute name */
#define	PROFILE_MAX_ATTRIBUTE_NAME_LEN	(128)
